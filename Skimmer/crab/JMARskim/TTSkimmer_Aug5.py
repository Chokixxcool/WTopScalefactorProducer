...
class TTbar_SemiLep(Module):
    ...
    def beginFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):
         ...
         self.out.branch("SelectedSubJet_phi",  "F")
    ...
    def analyze(self, event):
        ...
        electrons = [x for x in allelectrons if  x.cutBased > 2. and x.pt > self.minElpt  and abs(x.p4().Eta()) < self.maxMuEta   ]	 #loose pt cut for veto 
        muons     = [x for x in allmuons if x.pt > self.minMupt and x.mediumId and abs(x.p4().Eta()) < self.maxMuEta and x.pfRelIso03_all < 0.1] #loose pt cut for veto
        muons    .sort(key=lambda x:x.pt,reverse=True) # x.highPtId > 1
        ...
        try:
          if event.Flag_BadChargedCandidateFilter and event.Flag_BadPFMuonFilter and event.Flag_EcalDeadCellTriggerPrimitiveFilter and event.Flag_HBHENoiseFilter and event.Flag_HBHENoiseIsoFilter  and event.Flag_ecalBadCalibFilter and event.Flag_globalSuperTightHalo2016Filter and event.Flag_goodVertices:
        ...
        ### Check this list again for 2016 data: https://twiki.cern.ch/twiki/bin/viewauth/CMS/MissingETOptionalFiltersRun2#Moriond_2018 
        #if not passedMETFilters : continue
        ...
        if  WHadreco != None : 
            ...
            self.out.fillBranch("SelectedSubJet_eta",  WHadreco.phi)
